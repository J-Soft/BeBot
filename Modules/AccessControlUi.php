    ...
    */
    function command_handler($name, $msg, $origin)
    {
        if (preg_match("/^commands$/i", $msg)) {
            ...
            "/^commands update (gc|pgmsg|tell|extpgmsg|all) ([a-z01-9]+) ([a-zA-Z]+)$/i",
            $msg,
            $info
        )
        ) {
            ...
            "/^commands update (gc|pgmsg|tell|extpgmsg|all) ([a-z01-9]+) ([a-z01-9]+) ([a-zA-Z]+)$/i",
            $msg,
            $info
        )
        ) {
            ...
            "/^commands add (gc|pgmsg|tell|extpgmsg|all) ([a-z01-9]+) ([a-z01-9]+) ([a-zA-Z]+)$/i",
            $msg,
            $info
        )
        ) {
            ...
            "/^commands (del|rem) (gc|pgmsg|tell|extpgmsg|all) ([a-z01-9]+) ([a-z01-9]+)$/i",
            $msg,
            $info
        )
        ) {
        ...
        foreach ($rights as $channel => $value) {
            ...
            if (isset($this->bot->commands[$channel][$command])) {
                ...
                foreach ($value as $subcommand => $right) {
                    $blob .= "##highlight##{$command} {$subcommand}##end##:" . $this->Make_access_string(
                            $command . " " . $subcommand,
                            $right,
                            $channel
                        );
            ...
            if ($level !== "DELETED") {
                if ($current_level == $level) {
                    ...
                    $acsstr[] = $this->bot->core("tools")
                        ->chatcmd(
                            "commands update $channel $command " . $this->shortcuts[$level],
