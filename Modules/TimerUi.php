...
class TimerGUI extends BaseActiveModule
{
    ...
    function __construct(&$bot)
    {
        ...
        $this->bot->core("timer")
          ->create_class_setting("PublicTimer", "LowSpam",
    ...
    function command_handler($name, $msg, $channel)
    {
        ...
        Switch (strtolower($command[0])) {
            ...
            case 'rtimer':
                ...
                } elseif (preg_match("/^rtimer ([a-z]+ )?([0-9]+(:[0-9][0-9]){0,3}) ([0-9]+(:[0-9][0-9]){0,3}) (.*)/i",
                  $msg, $info)) {
                    if ($info[1] != "") {
                        $classused = $info[1];
                    }
    ...
    function show_timer($name, $channel)
    {
        ...
        $timers = $this->bot->db->select("SELECT * FROM #___timer WHERE " . $channelstr . $namestr . " ORDER BY endtime ASC",
          MYSQL_ASSOC);
        if (empty($timers)) {
