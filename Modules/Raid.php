...
class Raid extends BaseActiveModule
{
    ...
    function __construct(&$bot)
    {
        ...
        $this->bot->core("settings")
          ->create("Raid", "AnnounceDelay", 120, "Specify the delay between raid announces.",
        ...
        if ($this->bot->core("security")->check_access(
          ...
          ->core("settings")->get('Raid', 'Command')
            ...
            if (!$this->raid) {
                ...
                $this->pgleave = array();
                $this->start = time();
                $this->bot->send_output($name,
                  "##highlight##$name##end## has started the raid :: " . $this->clickjoin(), "both");
                $this->pause(true);
                $this->save();
                $this->register_event("cron", "1min");
          ...
          && !$this->bot->core("online")->in_chat($name)
            ...
            if ($this->raid) {
                ...
                $this->bot->send_output("",
                  "##highlight##$name##end## has ##highlight##joined##end## the raid :: " . $this->clickjoin(), "both");
    ...
    */
    ...
    function leave_raid($name)
    {
        if (!isset($this->user[$name])) {
            ...
            $this->bot->send_output("", "##highlight##$name##end## has ##highlight##left##end## the raid" . $ctj,
              "both");
                ...
                if (!empty($players)) {
                    ...
                    foreach ($players as $player) {
                        $who = $this->bot->core("whois")
                          ->lookup($player,
                            true); //All info about raiders are expected to be correct as already beeing member and all.
          ...
          ->core("settings")->get('Raid', 'Command')
            ...
            if (strtolower($lock) == "lock") {
                if ($this->locked) {
                    ...
                    $this->locked = true;
                    $this->bot->send_output("", "##highlight##$name##end## has ##highlight##locked##end## the raid.",
                      "both");
                    $this->save();
                ...
                if (!$this->locked) {
                    ...
                    $this->locked = false;
                    $this->bot->send_output("", "##highlight##$name##end## has ##highlight##unlocked##end## the raid.",
                      "both");
                    $this->save();
    ...
    */
        ...
        if ($this->bot->core("security")->check_access(
          ...
          ->core("settings")->get('Raid', 'Command')
            ...
            if ($this->raid) {
                ...
                if ($count > 0) {
                    $this->bot->send_output($name,
                      $name . " kicked ##highlight##" . implode(", ", $inside) . "##end## from privategroup.", "pgmsg");
                }
    ...
    function notify($name, $startup = false)
    {
        if (!$startup && $this->raid && !$this->locked) {
            ...
            $this->bot->send_tell($name,
              "Raid is running: ##highlight##" . $this->description . "##end##" . $move . " :: " . $this->clickjoin(true));
        }
    }
