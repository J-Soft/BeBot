    ...
    function __construct(&$bot)
    {
        ...
        $this->bot->core("settings")
            ->create(
                ...
                "Formating string for join notifications.",
                "",
                true
            );
        $this->bot->core("settings")
            ->create(
                "PrivGroup",
                "LeaveString",
                "#!NICKNAME!# has left #!BOTNAME!#",
                "Formating string for leave notifications.",
                "",
                true
            );
        $this->bot->core("settings")
    ...
    function kick_user($name, $type, $target = null)
    {
        ...
        if ($name == $target) {
            ...
            if ($name_access >= $target_access) {
                ...
                $this->bot->send_output(
                    $name,
                    "You cannot kick someone with a higher access level than yourself.",
                    "tell"
                );
            }
        } else {
