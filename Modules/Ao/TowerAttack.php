...
class TowerAttack extends BaseActiveModule
{
    ...
    function __construct(&$bot)
    {
        ...
        $this->bot->core("settings")
          ->create("TowerAttack", "AttacksPerBlob", 20, "How many attacks should be shown in the attack history blob?",
            "3;5;8;10;13;15;18;20");
        $this->bot->core("settings")
          ->create("TowerAttack", "VictoryBlobSize", 20,
            "How many victories from the history should be displayed at once?", "5;10;15;20");
        $this->bot->core("settings")
          ->create(
    ...
    function update_table()
    {
        ...
        switch ($this->bot->db->get_version("tower_result")) {
            case 1:
                $this->bot->db->update_table("tower_result", "zone", "add",
    ...
    function gmsg($name, $group, $msg)
    {
        ...
        if (preg_match(
          ...
          $msg, $info
            ...
            if (preg_match("/(.+) just attacked the (clan|neutral|omni) organization (.+)'s tower in (.+) at location \(([0-9]+), ([0-9]+)\)/i",
              $msg, $info)) {
                $infos["off_guild"] = "";
                $infos["off_side"] = "";
                ...
                if (preg_match("/(.+) (Clan|Omni|Neutral) organization (.+) attacked the (Clan|Omni|Neutral) (.+) at their base in (.+). The attackers won!!/i",
                  $msg, $info)) {
                    ...
                    if (preg_match("/The tower (.+) in (.+) was just reduced to (.+) % health by (.+) from the (.+) organization!$/i",
                      $msg, $info)) {
                        $this->relay_tower_damage($info[1], $info[2], $info[3], $info[4], $info[5]);
                    } else {
                        if (preg_match("/The tower (.+) in (.+) was just reduced to (.+) % health by (.+)!$/i", $msg,
                          $info)) {
                            $this->relay_tower_damage($info[1], $info[2], $info[3], $info[4]);
                        } else {
                            if (preg_match("/The tower (.+) in (.+) was just reduced to (.+) % health!$/i", $msg,
                              $info)) {
