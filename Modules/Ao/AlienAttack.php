    ...
    */
    function __construct(&$bot)
    {
        ...
        $this->bot->core("settings")
          ->create("AlienAttack", "Spam", "none", "Should the bot spam to gc or tells (on logon) or both?",
            "none;gc;tell;both");
        $this->bot->core("settings")
          ->create("AlienAttack", "Channel", "gc",
            "Into which channel should any output about alien attacks and city changes be send?", "gc;pgmsg;both");
        $this->bot->core("settings")
          ->create("AlienAttack", "PublicTimer", false,
            "Should a public timer in addition to the periodic spam be created on cloak up and down?");
        $this->bot->core("settings")
          ->create("AlienAttack", "CloakReminder", true,
    ...
    */
    ...
    function gmsg($name, $group, $msg)
    {
        if ($name == "0") {
            ...
            $action = "none";
            $player = "";
            if (preg_match("/Your radar station is picking up alien activity in the area surrounding your city./i",
              $msg)) {
                ...
                if (preg_match("/Your city in (.+) has been targeted by hostile forces./i", $msg, $info)) {
                    $action = "attack";
                    $zone = $info[1];
                    $this->bot->send_output("", "Our city in " . $zone . " is about to be under attack! 0MGZ RUN!!!!",
                      $channel);
                } else {
                    if (preg_match("/(.+) turned the cloaking device in your city off./i", $msg, $info)) {
                            ...
                            $this->bot->core("timer")
                              ->add_timer(
                                false, $player, 60 * 60 + 1, $this->bot
                                  ->core("shortcuts")
                                  ->get_short($this->bot->guildname) . "'s cloak can be enabled again", "gc", 0,
                                "CityWarning"
                              );
                        }
                        $this->bot->send_output("",
                        ...
                        if (preg_match("/(.+) turned the cloaking device in your city on./i", $msg, $info)) {
                            ...
                            $this->delete_cloak_reminder();
                            $this->bot->core("timer")
                              ->add_timer(false, "city", 60 * 60 + 1, $player . " cloakready", "internal", 0,
                                "CityCloakReady");
                            if ($this->bot->core("settings")
                              ->get("AlienAttack", "PublicTimer")
                            ) {
                                $this->bot->core("timer")
                                  ->add_timer(
                                    false, $player, 60 * 60 + 1, $this->bot
                                      ->core("shortcuts")
                                      ->get_short($this->bot->guildname) . "'s cloak can be disabled again", "gc", 0,
                                    "CityWarning"
                                  );
                            }
                            $this->bot->send_output("",
                            ...
                            if (preg_match("/(.+) initiated removal of the organization headquarters in (.+)/i", $msg,
                              $info)) {
                                $action = "HQ";
                                $player = $info[1];
                                $zone = $info[2];
                                ...
                                if (preg_match("/(.+) removed the organization headquarters in (.+)/i", $msg, $info)) {
                                    ...
                                    if (preg_match("/(.+) initiated removal of a (.+) in (.+)/i", $msg, $info)) {
                                        ...
                                        if (preg_match("/(.+) removed a (.+) in (.+)/i", $msg, $info)) {
                                            ...
                                            if (preg_match("/^The upkeep for your organization housing has not been paid./i",
                                              $msg)) {
                                                $action = "payment";
                                                $player = "";
                                            } else {
