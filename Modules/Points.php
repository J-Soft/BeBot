...
class Points extends BaseActiveModule
{
    ...
    */
    ...
    function check_alts($main)
    {
        ...
        if (!empty($alts)) {
            foreach ($alts as $alt) {
                $result = $this->bot->db->select("SELECT id, nickname, points FROM #___raid_points WHERE points != 0 AND id = " . $this->points_to($alt,
                    false));
                if (!empty($result)) {
                    $res = $result[0];
                    if ($res[0] != $this->points_to($res[1])) {
                ...
                $result = $this->bot->db->select("SELECT id FROM #___raid_points WHERE raiding = 1 and id = " . $this->points_to($alt,
                    false));
                if (!empty($result)) {
                    $res = $result[0];
