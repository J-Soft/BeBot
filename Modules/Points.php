...
class Points extends BaseActiveModule
{
    ...
    */
    function command_handler($name, $msg, $origin)
    {
        ...
        Switch (strtolower($msg[1])) {
            case 'give':
				if ($this->bot->exists_module("bidding") && !empty($this->bot->core("bidding")->bid)) {
					Return ("Error: giving point is ##highlight##forbidden##end## during a bid");
				} else {
					$this->give_points($name, $msg[2], $msg[3]);
				}			
                Break;
            case 'add':
                if (strlen($msg[4]) < 5) {
    ...
    function show_points($name, $target)
    {
        if (!$target || strtolower($target) == strtolower($name)) {
            ...
            if ($result) {
                if ($result[0][1] == "") {
                    $this->bot->db->query(
                        "UPDATE #___raid_points SET nickname = '" . $this->points_to_name(
                            $name
    ...
    */
    function tomain_points($name, $toggle)
    {
        ...
        if ($stat)
        {
			$this->bot->core("settings")->save("Alts", "Confirmation", true);
            ...
            foreach ($result as $res) {
                if ($res[0] != $this->points_to($res[1])) {
                    ...
                    if (empty($resu)) {
                        $this->bot->db->query(
                            "INSERT INTO #___raid_points (id, nickname, points, raiding) VALUES (" . $this->points_to(
                                $res[1]
                            ) . ", '" . $this->points_to_name($res[1]) . "', "
                            . $res[2] . ", 0)"
                        );
                    } else {
                        $this->bot->db->query(
                            "UPDATE #___raid_points SET points = " . ($res[2] + $resu[0][1]) . " WHERE id = " . $this->points_to(
                                $res[1]
                            )
                        );
                    }
        ...
        if ($check) {
            ...
            $this->bot->send_tell(
                $name,
                "Points going to the main character's account is now ##highlight##" . $txt . "##end##." . $add
            );
        }
    }
        ...
        if ($this->bot->core("settings")->get("Points", "Transfer")) {
            ...
            if ($num > ($result[0][0])) {
                ...
                if ($this->bot->core('player')->id($who) instanceof BotError) {
                    ...
                    $this->bot->db->query(
                        "UPDATE #___raid_points SET points = points - " . ($num) . " WHERE id = " . $this->points_to(
                            $name
                        )
                    );
                    $this->bot->db->query(
                        "INSERT INTO #___raid_points (id, nickname, points) VALUES (" . $this->points_to(
                            $who
                    ...
                    $this->bot->send_tell(
                        $name,
                        "You gave ##highlight##$num##end## raidpoints to ##highlight##$who##end##."
                    );
                    $this->bot->send_tell(
                        $who,
                        "You got ##highlight##$num##end## raidpoints from ##highlight##$name##end##."
                    );
                    return;
                }
            }
    ...
    function add_points($name, $who, $num, $why, $silent = false)
    {
        ...
        if ($this->bot->core('player')->id($who) instanceof BotError) {
            ...
            if (!$silent) {
                ...
                $this->bot->send_tell(
                    $name,
                    "You added ##highlight##$num##end## raidpoints to ##highlight##$who##end##'s account."
                );
                $this->bot->send_tell(
                    $who,
                    "##highlight##$name##end## added ##highlight##$num##end## raidpoints to your account.($why)"
                );
            }
    ...
    */
    function rem_points($name, $who, $num, $why, $silent = false)
    {
        ...
        if ($this->bot->core('player')->id($who) instanceof BotError) {
            ...
            $this->bot->db->query(
                "UPDATE #___raid_points SET points = points - " . ($num) . " WHERE id = " . $this->points_to($who)
            );
            if (!$silent) {
                $this->bot->send_output(
                    "",
                ...
                $this->bot->send_tell(
                    $name,
                    "You removed ##highlight##$num##end## raidpoints from ##highlight##$who##end##'s account."
                );
                $this->bot->send_tell(
                    $who,
    ...
    function view_log($name, $timeorname, $time2)
    {
        ...
        if ($ownlogs
            ...
            $logs = $this->bot->db->select(
                "SELECT name, points, by_who, time, why FROM #___raid_points_log WHERE $field " . $value . " ORDER BY time DESC, id"
            );
            if (!empty($logs)) {
