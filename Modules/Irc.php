...
class IRC extends BaseActiveModule
{
    ...
    */
    function __construct(&$bot)
    {
        ...
        $this->bot->core("settings")
          ->create("Irc", "IrcGuestPrefix", $pgroupprefix,
        ...
        $this->bot->core("settings")
          ->create("Irc", "ItemRef", "AOMainframe", "Should AO Mainframe of AUNO be used for links in item refs?",
            "AOMainframe;AUNO");
        $this->bot->core("settings")
          ->create("Irc", "Chat", $chatgroups, "Which channels should be relayed into IRC and vice versa?",
            "gc;pgroup;both");
        $this->bot->core("settings")
          ->create("Irc", "MaxRelaySize", 500, "What's the maximum amount of characters relayed to IRC?");
        $this->bot->core("settings")
          ->create("Irc", "NotifyOnDrop", false,
            "Should the chat be notified if something isn't relayed because it's too large?");
        $this->bot->core("settings")
          ->create("Irc", "UseGuildRelay", true,
            "Should chat coming from IRC also be relayed over the guild relay if it's set up?");
        $this->bot->core("settings")
          ->create("Irc", "Announce", true,
    ...
    function irc_connect($name = "")
    {
        ...
        $this->irc->registerActionhandler(SMARTIRC_TYPE_CHANNEL, $this->bot->commpre . 'online',
          $this->bot->commands["tell"]["irc"], 'irc_online');
        $this->irc->registerActionhandler(SMARTIRC_TYPE_CHANNEL, $this->bot->commpre . 'whois',
          $this->bot->commands["tell"]["irc"], 'irc_whois');
        $this->irc->registerActionhandler(SMARTIRC_TYPE_CHANNEL, $this->bot->commpre . 'is (.*)',
          $this->bot->commands["tell"]["irc"], 'irc_is');
        $this->irc->registerActionhandler(SMARTIRC_TYPE_CHANNEL, $this->bot->commpre . 'level (.*)',
          $this->bot->commands["tell"]["irc"], 'irc_level');
        $this->irc->registerActionhandler(SMARTIRC_TYPE_CHANNEL, $this->bot->commpre . 'lvl (.*)',
          $this->bot->commands["tell"]["irc"], 'irc_level');
        $this->irc->registerActionhandler(SMARTIRC_TYPE_CHANNEL, $this->bot->commpre . 'pvp (.*)',
          $this->bot->commands["tell"]["irc"], 'irc_level');
        $this->irc->registerActionhandler(SMARTIRC_TYPE_QUERY, $this->bot->commpre . 'level (.*)',
          $this->bot->commands["tell"]["irc"], 'irc_level');
        $this->irc->registerActionhandler(SMARTIRC_TYPE_QUERY, $this->bot->commpre . 'lvl (.*)',
          $this->bot->commands["tell"]["irc"], 'irc_level');
        $this->irc->registerActionhandler(SMARTIRC_TYPE_QUERY, $this->bot->commpre . 'pvp (.*)',
          $this->bot->commands["tell"]["irc"], 'irc_level');
        //$this -> irc -> registerActionhandler(SMARTIRC_TYPE_QUERY, $this -> bot -> commpre . 'command', $this -> bot -> commands["tell"]["irc"], 'command');
        $this->irc->registerActionhandler(SMARTIRC_TYPE_QUERY, $this->bot->commpre . 'is (.*)',
          $this->bot->commands["tell"]["irc"], 'irc_is');
        //$this -> irc -> registerActionhandler(SMARTIRC_TYPE_QUERY, $this -> bot -> commpre . 'tell (.*)', $this -> bot -> commands["tell"]["irc"], 'ao_msg');
        $this->irc->registerActionhandler(SMARTIRC_TYPE_QUERY, $this->bot->commpre . 'online',
          $this->bot->commands["tell"]["irc"], 'irc_online');
        $this->irc->registerActionhandler(SMARTIRC_TYPE_QUERY, $this->bot->commpre . 'whois',
          $this->bot->commands["tell"]["irc"], 'irc_whois');
        $this->irc->registerActionhandler(SMARTIRC_TYPE_QUERY, $this->bot->commpre . 'uid (.*)',
          $this->bot->commands["tell"]["irc"], 'irc_uid');
        $this->irc->registerActionhandler(SMARTIRC_TYPE_NAME, '.*', $this->bot->commands["tell"]["irc"], 'irc_query');
        $this->irc->registerActionhandler(SMARTIRC_TYPE_CHANNEL, '.*', $this->bot->commands["tell"]["irc"],
          'irc_receive');
        $this->irc->registerActionhandler(SMARTIRC_TYPE_JOIN, '.*', $this->bot->commands["tell"]["irc"], 'irc_join');
        $this->irc->registerActionhandler(SMARTIRC_TYPE_NICKCHANGE, '.*', $this->bot->commands["tell"]["irc"],
          'irc_nick');
        ...
        $this->irc->registerActionhandler(SMARTIRC_TYPE_QUERY, '.*', $this->bot->commands["tell"]["irc"],
          'irc_receive_msg');
    ...
    function irc_receive(&$irc, &$data)
    {
        if ((strtolower($data->message) != strtolower(str_replace("\\", "", $this->bot->commpre . 'online')))
          ...
          && (strtolower($data->message) != strtolower(str_replace("\\", "", $this->bot->commpre . 'level')))
            ...
            $msg = preg_replace(
              "/" . chr(3) . chr(3) . "(.+?)" . chr(3) . " " . chr(3) . "\((.+?)id=([0-9]+)&id2=([0-9]+)&ql=([0-9]+)\)" . chr(3) . chr(3) . "/iU",
              $itemstring, $msg
            );
            $msg = preg_replace(
              "/" . chr(3) . chr(3) . "(.+?)" . chr(3) . " " . chr(3) . "\((.+?)LowID=([0-9]+)&HiID=([0-9]+)&QL=([0-9]+)\)" . chr(3) . chr(3) . "/iU",
              $itemstring, $msg
            );
            ...
            if (preg_match("/" . chr(2) . chr(2) . chr(2) . "(.+)" . chr(2) . "(.+)/i", $msg, $info)) {
                ...
                $txt = "##irc_group##" . $this->bot->core("settings")
                    ->get("Irc",
                      "Guildprefix") . "##end## ##irc_user##" . $data->nick . ":##end####irc_text## " . $msg . "##end##";
            }
        ...
        if (($data->nick != $this->bot->core("settings")
          ...
          && (strtolower(
            ...
            $msg = "##irc_group##" . $this->bot->core("settings")
                ->get("Irc",
                  "Guildprefix") . "##end## ##highlight##" . $data->nick . "##end## has logged##highlight## on##end##.";
            $this->bot->send_output(
    ...
    function irc_part(&$irc, &$data)
    {
        if (($data->nick != $this->bot->core("settings")
          ...
          && (strtolower(
            ...
            $msg = "##irc_group##" . $this->bot->core("settings")
                ->get("Irc",
                  "Guildprefix") . "##end## ##highlight##" . $data->nick . "##end## has logged##highlight## off##end## (" . $data->message . ").";
            $this->bot->send_output(
              "", $msg, $this->bot->core("settings")
              ->get("Irc", "AnnounceTo")
            );
    ...
    */
    ...
    function irc_nick(&$irc, &$data)
    {
        ...
        $txt = "##irc_group##" . $this->bot->core("settings")
            ->get("Irc",
              "Guildprefix") . "##end## ##irc_user##" . $data->nick . "##end####irc_text## is known as##end## ##irc_user##" . $data->message . "##end##";
        if (strtolower(
            $this->bot->core("settings")
              ->get("Irc", "AnnounceTo")
          ) != "none"
