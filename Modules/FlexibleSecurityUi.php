...
class FlexibleSecurityGUI extends BaseActiveModule
{
    ...
    function command_handler($name, $msg, $origin)
    {
        ...
        } elseif (preg_match("/^flexible condition add ([a-z_]+) (level|rank_id|at_id) (<|<=|>|>=|=|!=) ([01-9]+)$/i",
          $msg, $info)) {
        ...
        } elseif (preg_match("/^flexible condition add ([a-z_]+) faction (=|!=) (omni|clan|neutral|all)$/i", $msg,
          $info)) {
        ...
        } elseif (preg_match("/^flexible condition del ([a-z_]+) (level|rank_id|at_id|profession|faction|org_id) (<|<=|>|>=|=|!=) (.+)/i",
          $msg, $info)) {
            ...
            foreach ($grps as $grp) {
                ...
                if (empty($conds)) {
                    ...
                    foreach ($conds as $cond) {
                        ...
                        $blob .= " ";
                        $blob .= $this->bot->core("tools")
                          ->chatcmd("flexible condition del " . $grp[1] . " " . $cond[0] . " " . htmlentities($cond[1]) . " " . $cond[2],
                            "[DELETE]");
                    }
                }
            }
