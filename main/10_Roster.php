	...
	function gmsg($name, $group, $msg)
	{
		if ($name == "0")
		{
			if (preg_match("/(.+) kicked (.+) from the organization./i", $msg, $info))
			{
				...
				$this->bot->send_irc($this->bot->core("settings")->get("Irc", "Ircguildprefix"), "", "$person has been Kicked from Org by $source");
			}
			else if (preg_match("/(.+) has left the organization./i", $msg, $info))
			{
				...
				$this->bot->send_irc($this->bot->core("settings")->get("Irc", "Ircguildprefix"), "", "$person has Left the Org");
			}
			else if (preg_match("/(.+) invited (.+) to your organization./i", $msg, $info))
			{
				...
				$this->bot->send_irc($this->bot->core("settings")->get("Irc", "Ircguildprefix"), "", "$person has been invited to the org by $inviter");
			}
		}
	}
