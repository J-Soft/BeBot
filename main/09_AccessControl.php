...
class AccessControl_Core extends BasePassiveModule
{
	...
	function update_table()
	{
		...
		{
			$this->bot->db->set_version("access_control", $this->bot->core("settings")->get("accesscontrol", "schemaversion"));
		...
		switch ($this->bot->db->get_version("access_control"))
		{
			case 1:
				...
				$this->bot->db->update_table("access_control", array("command" , "subcommand" , "channel"), "alter", "ALTER IGNORE TABLE #___access_control ADD PRIMARY KEY (command, subcommand, channel)");
			...
			for ($i = 0; $i < $channelcount; $i ++)
			{
				if (! (empty($this->bot->commands[$channel[$i]])))
				{
					...
					foreach ($this->bot->commands[$channel[$i]] as $key => $val)
					{
						...
						if (empty($rights))
						{
							$this->bot->db->query("INSERT INTO #___access_control (command, subcommand, channel, " . "minlevel) VALUES ('" . $key . "', '*', '" . $channel[$i] . "', '" . $this->bot->core("settings")->get("Accesscontrol", "Defaultlevel") . "')");
						}
					}
				}
			}
	...
	function do_check($user, $command, $subcommand, $channel)
	{
		...
		else
		{
			...
			return $this->bot->core("security")->check_access($user, $this->access_cache[strtolower($command)][strtolower($subcommand)][strtolower($channel)]);
		}
	}
	...
	function check_rights($user, $command, $msg, $channel)
	{
		...
		if (($channel == "gc" && $this->bot->core("settings")->get("AccessControl", "LockGc")) || ($channel == "pgmsg" && $this->bot->core("settings")->get("AccessControl", "LockPgroup")))
		{
			return FALSE;
		}
	...
	function save($name, $desc, $update = FALSE)
	{
		...
		$lshort = array("ANONYMOUS" => "AN" , "GUEST" => "G" , "MEMBER" => "M" , "LEADER" => "L" , "ADMIN" => "A" , "SUPERADMIN" => "SA" , "OWNER" => "O" , "DISABLED" => "D");
	...
	function load($name)
	{
		...
		$llong = array('AN' => 'ANONYMOUS' , 'G' => 'GUEST' , 'M' => 'MEMBER' , 'L' => 'LEADER' , 'A' => 'ADMIN' , 'SA' => 'SUPERADMIN' , 'O' => 'OWNER' , 'D' => 'DISABLED');
