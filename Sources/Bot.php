...
class Bot
{
	...
	*/
	...
	function handle_command_input($user, $msg, $channel, $pgname = NULL)
	{
		...
		if (! empty($this->commands[$channel]))
		{
			...
			if ($channel == "tell" && ! $this->core("settings")->get("Core", "RequireCommandPrefixInTells") && $this->commpre != "" && $msg[0] != $stripped_prefix)
			{
	...
	*/
	function inc_pgmsg($args)
	{
		...
		if (strtolower($this->botname) == strtolower($user))
		{
			...
			{
				$this->log("PGRP", "MSG", "[" . $this->core("player")->name($args[0]) . "] " . $user . ": " . $args[2]);
			}
			return;
		}
		else
	...
	*/
	...
	public function register_event($event, $target, &$module)
	{
		...
		$events = array('connect' , 'disconnect' , 'pgjoin' , 'pginvite' , 'pgleave' , 'extpgjoin' , 'extpgleave' , 'cron' , 'settings' , 'timer' , 'logon_notify' , 'buddy' , 'privgroup' , 'gmsg' , 'tells' , 'extprivgroup' , 'irc');
	...
	{
		$event = strtolower($event);
		$events = array('connect' , 'disconnect' , 'pgjoin' , 'pginvite' , 'pgleave' , 'extpgjoin' , 'extpgleave' , 'cron' , 'settings' , 'timer' , 'logon_notify' , 'buddy' , 'privgroup' , 'gmsg' , 'tells' , 'extprivgroup');
		if (in_array($event, $events))
		{
			if ($event == 'gmsg')
			{
