...
public $bothandle;
    ...
    private $cron_job_timer = array();
        private $cron_job_active = array(); // == botname@dimension
    ...
    public static function factory($config_file = null)
    {
        ...
        self::$instance[$bothandle]->ini = ConfigMagik::get_instance($bothandle,
          "conf/" . ucfirst(strtolower($bot_name)) . ".Modules.ini", true, true);
    ...
    function load_files($section, $directory)
    {
        ...
        while ($module = $folder->read()) {
            ...
            if (!is_dir($module) && !preg_match("/^_/", $module) && preg_match("/\.php$/i",
    ...
    */
    ...
    function connect()
    {
        ...
        */
        ...
        $this->core("settings")
          ->create("Core", "CommandErrorGc", false,
        ...
        $this->core("settings")
          ->create("Core", "DisablePGMSGchat", $dispg,
    ...
    */
    ...
    function inc_tell($args)
    {
        ...
        if (preg_match("/is AFK .Away from keyboard./i", $args[1]) || preg_match("/.tell (.+)help/i",
            $args[1]) || preg_match("/I only listen to members of this bot/i", $args[1])
