...
class AOChat
{
	...
	*/
	function authenticate($username, $password)
	{
		...
		$pak = new AOChatPacket("out", AOCP_LOGIN_REQUEST, array(0 , $username , $key), $this->game);
		...
		{
			$this->chars[] = array("id" => $packet->args[0][$i] , "name" => ucfirst(strtolower($packet->args[1][$i])) , "level" => $packet->args[2][$i] , "online" => $packet->args[3][$i]);
		}
		$this->username = $username;
...
//		array_unshift($stack, array('user' => $u , 'timeout' => $timelimit));
	...
	function send_group($group, $msg, $blob = "\0")
	{
		...
		return $this->send_packet(new AOChatPacket("out", AOCP_GROUP_MESSAGE, array($gid , $msg , $blob), $this->game));
	}
	...
	function group_join($group)
	{
		...
		return $this->send_packet(new AOChatPacket("out", AOCP_GROUP_DATA_SET, array($gid , $this->grp[$gid] & ~ AOC_GROUP_MUTE , "\0"), $this->game));
	}
	...
	function send_privgroup($group, $msg, $blob = "\0")
	{
		...
		return $this->send_packet(new AOChatPacket("out", AOCP_PRIVGRP_MESSAGE, array($gid , $msg , $blob), $this->game));
	}
