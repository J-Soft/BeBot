...
class Net_SmartIRC_messagehandler extends Net_SmartIRC_irccommands
{
    ...
    function _event_join(&$ircdata)
    {
        if ($this->_channelsyncing == true) {
            ...
            $this->log(
                SMARTIRC_DEBUG_CHANNELSYNCING,
                'DEBUG_CHANNELSYNCING: ' . $ircdata->nick . ' joins channel: ' . $ircdata->channel,
                __FILE__,
                __LINE__
            );
    ...
    function _event_mode(&$ircdata)
    {
        ...
        if ($ircdata->rawmessageex[2] == $this->_nick) {
            ...
            if ($this->_channelsyncing == true) {
                ...
                $this->log(
                    SMARTIRC_DEBUG_CHANNELSYNCING,
                    'DEBUG_CHANNELSYNCING: updating channel mode for: ' . $channel->name,
                    __FILE__,
                    __LINE__
                );
                    ...
                    switch ($mode[$i]) {
                        ...
                        case 'k':
                            ...
                            if ($add) {
                                $this->log(
                                    SMARTIRC_DEBUG_CHANNELSYNCING,
                                    'DEBUG_CHANNELSYNCING: stored channel key for: ' . $channel->name,
                                    __FILE__,
                                    __LINE__
                                );
                                $channel->key = $key;
                            ...
                            if ($remove) {
                                $this->log(
                                    SMARTIRC_DEBUG_CHANNELSYNCING,
                                    'DEBUG_CHANNELSYNCING: removed channel key for: ' . $channel->name,
                                    __FILE__,
                                    __LINE__
                                );
                                $channel->key = '';
