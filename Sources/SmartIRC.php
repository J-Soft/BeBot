...
class Net_SmartIRC_base
{
		...
		if ($this->_state() == SMARTIRC_STATE_CONNECTED)
		{
			...
			while (count($rawdataar) > 0)
			{
				...
				if (substr($rawline, 0, 1) == ':')
				{
					...
					if ($ircdata->type & (SMARTIRC_TYPE_CHANNEL | SMARTIRC_TYPE_ACTION | SMARTIRC_TYPE_MODECHANGE | SMARTIRC_TYPE_KICK | SMARTIRC_TYPE_PART | SMARTIRC_TYPE_JOIN))
					{
						$ircdata->channel = $lineex[2];
					}
					else if ($ircdata->type & (SMARTIRC_TYPE_WHO | SMARTIRC_TYPE_BANLIST | SMARTIRC_TYPE_TOPIC | SMARTIRC_TYPE_CHANNELMODE))
					{
					...
					$this->log(SMARTIRC_DEBUG_MESSAGEPARSER, 'DEBUG_MESSAGEPARSER: ircdata nick: "' . $ircdata->nick . '" ident: "' . $ircdata->ident . '" host: "' . $ircdata->host . '" type: "' . $ircdata->type . '" from: "' . $ircdata->from . '" channel: "' . $ircdata->channel . '" message: "' . $ircdata->message . '"', __FILE__, __LINE__);
				}
	...
	function _handlemessage($messagecode, &$ircdata)
	{
		...
		{ // its not numericcode so already a name/string
