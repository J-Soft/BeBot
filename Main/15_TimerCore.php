...
class Timer_Core extends BasePassiveModule
{
    ...
    function __construct(&$bot)
    {
        ...
        $this->bot->core("settings")
          ->create("Timer", "DefaultClass", "Standard",
        ...
        $this->bot->core("settings")
          ->create("Timer", "DeleteRank", "ADMIN", "Minimal access level to remove timer of other players.",
        ...
        $this->bot->core("settings")
          ->create("Timer", "RecoveryCheckInterval", 10,
        ...
        $classid = $this->create_timer_class("LowSpam",
        ...
        $nextid = $this->create_timer_class_entry($classid, $nextid, 1800, "Timer", "has 30 minutes left");
        $nextid = $this->create_timer_class_entry($classid, $nextid, 3600, "Timer", "has one hour left");
        $classid = $this->create_timer_class("None",
    ...
    function create_class_cache()
    {
        ...
        $classentries = $this->bot->db->select("SELECT id, class_id, next_id, notify_delay, notify_prefix, notify_suffix FROM #___timer_class_entries",
          MYSQL_ASSOC);
        ...
        $settings_entries = $this->bot->db->select("SELECT id, name, current_class FROM #___timer_class_settings",
          MYSQL_ASSOC);
            ...
            foreach ($timers as $timer) {
                ...
                elseif ($timer['repeatinterval'] > 0) {
                    ...
                    if ($this->class_cache[$timerclass]['class'] == "Default") {
                        ...
                        $entry = $this->get_class_entry($this->class_cache[$timerclass]['class'],
    ...
    {
        return $this->bot->db->select("SELECT id, name FROM #___timer WHERE channel = 'internal' AND owner = '" . $owner . "'",
          MYSQL_ASSOC);
    }
    ...
    function create_class_setting($name, $default_class_name, $description)
    {
        ...
        $class_setting = $this->bot->db->select("SELECT id, current_class FROM #___timer_class_settings WHERE name = '" . mysql_real_escape_string($name) . "'",
          MYSQL_ASSOC);
    ...
    {
        $class_setting = $this->bot->db->select("SELECT id FROM #___timer_class_settings WHERE name = '" . mysql_real_escape_string($name) . "'",
          MYSQL_ASSOC);
        if (!empty($class_setting)) {
    ...
    function get_class_setting($name)
    {
        ...
        $cname = $this->bot->db->select("SELECT name FROM #___timer_classes WHERE id = " . $this->settings_cache[strtolower($name)],
          MYSQL_ASSOC);
        if (empty($cname)) {
            return "";
        }
