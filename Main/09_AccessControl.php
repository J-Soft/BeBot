...
class AccessControl_Core extends BasePassiveModule
{
    ...
    function __construct(&$bot)
    {
        ...
        */
        ...
        $this->bot->core("settings")
          ->create("AccessControl", "LockGc", false, "Are all commands in the guild chat locked from use?", "On;Off",
            true);
        $this->bot->core("settings")
          ->create("AccessControl", "LockPgroup", false, "Are all commands in the private chatgroup locked from use?",
            "On;Off", true);
        $this->update_table();
    ...
    function do_check($user, $command, $subcommand, $channel)
    {
        ...
        if (in_array($this->access_cache[strtolower($command)][strtolower($subcommand)][strtolower($channel)],
          $this->deny_levels)) {
            return false;
        } else {
