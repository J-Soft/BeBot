    ...
    */
    ...
    { // Start function connect()
        ...
        $this->bot->core("settings")
          ->create("Security", "orggov", "Unknown", "Orginization Government Form",
                ...
                switch ($vars[1]) {
                    case "changelevel":
                        if ($this->check_access($source, "SUPERADMIN")) {
                            ...
                            if (preg_match("/^security changelevel (Department) (Squad|Unit|Board) (Commander|Leader|Member) (.+)$/i",
                              $msg, $info)) {
                                return $this->change_level($info[2] . " " . $info[3], $info[4], $info[1]); // SA
                            } elseif (preg_match("/^security changelevel (Faction) (Board Member) (.+)$/i", $msg,
                              $info)) {
                        ...
                        if (preg_match("/^admin group add ([a-zA-Z0-9]+)$/i", $msg, $info)) {
                            ...
                            if (preg_match("/^admin group (remove|rem|del) ([a-zA-Z0-9]+)$/i", $msg, $info)) {
                                ...
                                if (preg_match("/^admin add ([a-zA-Z0-9]+) ([a-zA-Z0-9]+)$/i", $msg, $info)) {
                                    ...
                                    if (preg_match("/^admin (remove|rem|del) ([a-zA-Z0-9]+) ([a-zA-Z0-9]+)$/i", $msg,
                                      $info)) {
                                        if ($this->check_access(
                                          $source, $this->bot
                                          ->core("settings")->get('Security', 'Remgroupmember')
            ...
            if (!empty($result)) // This really should never be empty as this module automaticaly inserts data here.
            {
                foreach ($result as $rank) {
                    if ($rank['access_level'] == SUPERADMIN) {
                        $superadmin
                          .= "+ Org Rank " . $rank['org_rank'] . ":\n    Change " . $rank['org_rank'] . " Access Level To: " . $this->change_links(SUPERADMIN,
                            $rank, $msgtype)
                          . "\n";
                    }
                    if ($rank['access_level'] == ADMIN) {
                        $admin
                          .= "+ Org Rank " . $rank['org_rank'] . ":\n    Change " . $rank['org_rank'] . " Access Level To: " . $this->change_links(ADMIN,
                            $rank, $msgtype) . "\n";
                    }
                    if ($rank['access_level'] == LEADER) {
                        $leader
                          .=
                          "+ Org Rank " . $rank['org_rank'] . ":\n    Change " . $rank['org_rank'] . " Access Level To: " . $this->change_links(LEADER,
                            $rank, $msgtype) . "\n";
                    }
                    if ($rank['access_level'] == MEMBER) {
                        $member
                          .=
                          "+ Org Rank " . $rank['org_rank'] . ":\n    Change " . $rank['org_rank'] . " Access Level To: " . $this->change_links(MEMBER,
                            $rank, $msgtype) . "\n";
                    }
                    if ($rank['access_level'] == GUEST) {
                        $guest
                          .= "+ Org Rank " . $rank['org_rank'] . ":\n    Change " . $rank['org_rank'] . " Access Level To: " . $this->change_links(GUEST,
                            $rank, $msgtype) . "\n";
                    }
                    if ($rank['access_level'] == ANONYMOUS) {
                        $anonymous
                          .= "+ Org Rank " . $rank['org_rank'] . ":\n    Change " . $rank['org_rank'] . " Access Level To: " . $this->change_links(ANONYMOUS,
                            $rank, $msgtype)
                          . "\n";
                    }
                }
...
function add_group($groupname, $description, $caller = "Internal Process")
    { // Start function add_group()
        ...
        $this->bot->log("SECURITY", "ADDGROUP",
...
function del_group($target, $caller = "Internal Process")
    { // Start function del_group()
        ...
        $this->bot->db->query($sql);
        $this->bot->log("SECURITY", "DELGROUP",
