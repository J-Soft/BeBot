    ...
    */
    ...
    function read_theme()
    {
        ...
        foreach ($theme_file as $theme_line) {
            ...
            if (preg_match("/color name=\"([a-z_]+)\" code=\"(#[0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f])\"/i",
              $theme_line, $info)) {
            ...
            } elseif (preg_match("/theme name=\"(.*)\" version=\"(.*)\" author=\"(.*)\" link=\"(.*)\"/i", $theme_line,
              $info)) {
    ...
    function read_scheme_file($filename)
    {
        ...
        foreach ($scheme_file as $scheme_line) {
            if (preg_match("/scheme module=\"([a-z_]+)\" name=\"([a-z_]+)\" code=\"([a-z]+)\"/i", $scheme_line,
              $info)) {
                $this->bot->db->query(
