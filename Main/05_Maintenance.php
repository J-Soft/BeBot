    ...
    function connect()
    {
        ...
        $this->bot->core("settings")
            ->create(
                ...
                "info",
                "",
                "Info saved while restarting, blank when not doing maintenance.",
                null,
                true,
                2
            );
                            ...
                            Switch ($info[3]) {
                                ...
                                Default:
                                    $this->bot->send_output(
                                        $info[1],
                                        "Error: Unknown Step number: " . $info[3],
                                        $info[2]
                                    );
                                    break;
                            }
            ...
            case 'settings':
                ...
                if (!empty($olddata)) {
                    ...
                    if (!$inside) {
                        ...
                        $this->bot->send_output(
                            $name,
                            "Maintenance ToDo list: " . $this->bot
                                ->core("tools")
                                ->make_blob("Click to view", $inside),
                            $origin
                        );
                ...
                if (!isset($new[strtolower($mod)][strtolower($set)])) {
                    ...
                    if ($compare) {
                        foreach ($compare as $id => $name) {
                            if (!isset($this->dontupdate[$mod][$set][$name]) && $value[$id] != $new[strtolower(
                                    $mod
                                )][strtolower($set)][$id]
                            ) {
                                $this->update[] = array(
        ...
        if (!empty($this->update)) {
            foreach ($this->update as $u) {
                $inside .= $u[0] . " => " . $u[1] . ":\n    " . $u[2] . ": " . $u[3] . " => " . $u[4] . "  " . $this->bot
                        ->core("tools")
                        ->chatcmd(
                            "maintenance settings dont update " . $u[0] . " " . $u[1] . " " . $u[2],
                            "[Dont Change]"
                        ) . "\n";
            }
        }
                ...
                if ($info != "") {
                    ...
                    if (!empty($this->update)) {
                        foreach ($this->update as $up) {
                            $this->bot->db->query(
                                "UPDATE #___settings SET " . $up[2] . " = '" . $up[4] . "' WHERE module = '" . $up[0] . "' AND setting = '" . $up[1] . "'"
                            );
                        }
                    }
                    $this->bot->core("settings")
