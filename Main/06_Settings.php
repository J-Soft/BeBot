...
{ // Start Class
    ...
    { // Start function save()
        ...
        if (!isset($this->settings_cache[strtolower($module)][strtolower($setting)])) {
            if (!is_null($this->settings_cache[strtolower($module)][strtolower($setting)])) {
                $this->error->set(
                    "Setting " . $setting . " for module " . $module . " could not be saved. " . $setting . " does not exist."
                );
                return $this->error;
            }
        }
        ...
        if ($datatype == "unknown") {
            $this->error->set(
                "Setting " . $setting . " for module " . $module . " could not be saved. Datatype is unknown or not supported."
            );
            return $this->error;
        ...
        $module_sql = $this->bot->db->real_escape_string(strval($module));
        $setting_sql = $this->bot->db->real_escape_string(strval($setting));
        $value_sql = $this->bot->db->real_escape_string(strval($value));
        if ($noupdate) {
            $sql
                =
        ...
        if ($result) {
            ...
            $this->error->set(
                "Could not save setting " . stripslashes($setting) . " for module " . stripslashes(
                    $module
                ) . " to database. SQL: " . $sql
            );
            $this->change_user = "";
    ...
    */
    function create(
        ...
        $module = $this->bot->db->real_escape_string($module);
        $setting = $this->bot->db->real_escape_string($setting);
        $valuesql = $this->bot->db->real_escape_string($value);
        $longdesc = $this->bot->db->real_escape_string($longdesc);
        $defaultoptions = $this->bot->db->real_escape_string($defaultoptions);
        ...
        if (!empty($check)) {
            $check = $check[0];
            if ($check[0] != stripslashes($longdesc) || $check[1] != stripslashes(
                    $defaultoptions
                ) || $check[2] != stripslashes($hidden) || $check[3] != stripslashes($disporder)
            ) {
    ...
    */
    ...
    { // Start function update()
        ...
        $to = $this->bot->db->real_escape_string($to);
        ...
        if ($this->get("Settings", "Log")) {
            $this->bot->log(
                "SETTINGS",
                "UPDATE",
                "Set " . stripslashes($what) . " for setting [" . stripslashes($module) . "][" . stripslashes(
                    $setting
                ) . "] to " . stripslashes($to)
            );
        }
    ...
    { // Start function del()
        ...
        if (is_null($setting)) // Delete a setting group.
        {
            ...
            if (!isset($this->settings_cache[strtolower($module)][strtolower($setting)])) {
                ...
                $sql = "DELETE FROM #___settings WHERE module = '" . $this->bot->db->real_escape_string(
                        $module
                    ) . "' AND setting = '" . $this->bot->db->real_escape_string($setting) . "'";
                $this->bot->db->query($sql);
    ...
    */
    function update_settings_table()
    {
        ...
        Switch ($schemaversion) {
            case 1:
				$col = $this->bot->db->select("SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = '#___settings' AND COLUMN_NAME = 'disporder'");
				if(count($col)==0) {
