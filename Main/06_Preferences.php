    ...
    */
    ...
    function update_table()
    {
        ...
        Switch ($this->bot->db->get_version("preferences")) {
            case 1:
                $this->bot->db->update_table("preferences", "owner", "alter",
                  "ALTER TABLE #___preferences MODIFY owner BIGINT NOT NULL");
            ...
            if ($prefs[1] != stripslashes($description) || $prefs[2] != $possible_values) {
                ...
                if ($prefs[2] != $possible_values) {
                    ...
                    if (!isset($pv[strtolower($prefs[3])])) // current default invalid, reset
                    {
                        ...
                        $this->bot->log("PREFS", "UPDATED",
                          "Reset default value as it was invalid for " . stripslashes($name) . " for module " . stripslashes($module));
                    }
                    ...
                    if (!empty($uprefs)) {
                        ...
                        if ($count > 0) {
                            $this->bot->log("PREFS", "UPDATED",
                              "Reset $count user prefs as they were invalid for " . stripslashes($name) . " for module " . stripslashes($module));
                        }
                    }
                }
            }
    ...
    */
    ...
    function get($name, $module = false, $setting = false)
    {
        ...
        if ($module != false && $setting == false) {
            ...
            $prefs = array_merge($this->cache['def'][strtolower($module)],
