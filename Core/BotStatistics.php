...
class BotStatistics_Core extends BasePassiveModule
{
    ...
    function check_bots($name, $origin, $bot = false, $dim = false)
    {
        ...
        if ($bot) {
            $bot = $this->bot->db->real_escape_string($bot);
            $dim = $this->bot->db->real_escape_string($dim);
            $result = $this->bot->db->select(
            ...
            if (!empty($result)) {
                ...
                $log = $this->bot->db->select(
                    "SELECT start, end FROM " . $this->DB . "#___bots_log WHERE bot = '" . $bot[0] . "' AND dim = '" . $bot[1] . "'"
                );
                $day = 60 * 60 * 24;
                $daytime = time() - $day;
                if ($daytime < $bot[4]) {
    ...
    function cron($cron)
    {
        $this->online = true;
        $this->bot->db->query(
            "UPDATE " . $this->DB . "#___bots SET time = '" . time(
            ) . "' WHERE bot = '" . $this->bot->botname . "' AND dim = '" . $this->bot->dimension . "'"
        );
        if ($cron == 86400) {
