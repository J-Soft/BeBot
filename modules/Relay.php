...
class Relay extends BaseActiveModule
{
  ...
  */
  function tell($name, $msg)
  {
    ...
    elseif (strtolower($input['com']) == 'gcr' && $this->bot->core("settings")->get('Relay', 'Status') && (($this->bot->core("settings")->get("Relay", "Strictnamecheck") && strtolower($this->bot->core("settings")->get("Relay", "Relay")) == strtolower($name)) || !($this->bot->core("settings")->get("Relay", "Strictnamecheck"))))
    {
      $txt = $input['args'];
  ...
  function extpgmsg($pgroup, $name, $msg, $db = FALSE)
  {
    ...
    elseif (strtolower($input['com']) == 'gcr' && $this->bot->core("settings")->get('Relay', 'Status') && ($db || strtolower($this->bot->core("settings")->get('Relay', 'Relay')) == strtolower($pgroup)))
    {
      $txt = $input['args'];
      $txt = explode(" ", $txt, 2);
      if ($txt[0] == '&$enc$&') {
  ...
  */
  function relay_to_pgroup($name, $msg, $type = "notchat")
  {
    if ($this->bot->core("settings")->get('Relay', 'Status')) {
      ...
      if ($this->bot->core("settings")->get("Relay", "Relay") != '' && ($this->bot->core("settings")->get('Relay', 'Org') == "Both" || $this->bot->core("settings")->get('Relay', 'Org') == "Relaybots")) {
  ...
  */
  function relay_to_gc($name, $msg)
  {
    if ($this->bot->core("settings")->get('Relay', 'Status')) {
      ...
      if ($this->bot->core("settings")->get("Relay", "Relay") != '' && ($this->bot->core("settings")->get('Relay', 'Priv') == "Both" || $this->bot->core("settings")->get('Relay', 'Priv') == "Relaybots")) {
  ...
  function extpgjoin($pgname, $name, $cron = FALSE)
  {
    ...
    if ($this->bot->core("settings")->get('Relay', 'Status') && ($cron || strtolower($this->bot->core("settings")->get('Relay', 'Relay')) == strtolower($pgname))) {
