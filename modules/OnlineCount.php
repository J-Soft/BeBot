...
class OnlineCounting extends BaseActiveModule
{
	...
	{
		return $this->bot->db->select("SELECT DISTINCT(t1.nickname), t2.level, t2.defender_rank_id" . " FROM " . $this->bot->core("online")->full_tablename() . " WHERE t2.org_name = '" . $orgname . "' ORDER BY t1.nickname ASC");
	}
	...
	function get_prof($prof)
	{
		...
		return $this->bot->db->select("SELECT DISTINCT(t1.nickname), t2.level, t2.defender_rank_id" . " FROM " . $this->bot->core("online")->full_tablename() . " WHERE t2." . $this->cp . " " . $profsearch . " ORDER BY t1.nickname ASC");
	}

	function get_orgs()
	{
		$innersql = "SELECT t2.org_name as org, COUNT(DISTINCT t1.nickname) AS count " . " FROM " . $this->bot->core("online")->full_tablename() . " WHERE t2.org_name != '' GROUP BY t2.org_name ORDER BY count DESC, org_name ASC";
		$sql = "SELECT t1.org AS org, t1.count AS count, SUM(t2.level) / t1.count AS avg_level FROM (" . $innersql . ") AS t1, #___whois AS t2, #___online AS t3 WHERE t1.org = t2.org_name AND " . "t2.nickname = t3.nickname AND " . $this->bot->core("online")->otherbots("t3.") . " AND " . $this->bot->core("online")->channels("t3.") . " GROUP BY org ORDER BY t1.count DESC, t1.org ASC";
	...
	function count_all()
	{
		...
		$query = "SELECT t2." . $this->cp . " as profession, COUNT(DISTINCT t1.nickname) as count" . " FROM " . $this->bot->core("online")->full_tablename() . " WHERE t2." . $this->cp . " IN (" . $profession_list . ") GROUP BY " . $this->cp . "";
	...
	function count($shortcut)
	{
		if (($prof = $this->bot->core("professions")->full_name($shortcut)) instanceof BotError)
			return $prof;
		$pcount = $this->bot->db->select("SELECT COUNT(DISTINCT t1.nickname) FROM " . $this->bot->core("online")->full_tablename() . " WHERE t2." . $this->cp . " = '" . $prof . "'");
		if ($pcount[0][0] == 0)
	...
	function count_org()
	{
		...
		$tcount = $this->bot->db->select("SELECT count(DISTINCT nickname) as count FROM #___online WHERE " . $this->bot->core("online")->otherbots("") . " AND " . $this->bot->core("online")->channels(""), MYSQL_ASSOC);
	...
	{
		$pcount = $this->bot->db->select("SELECT COUNT(DISTINCT t1.nickname) FROM " . $this->bot->core("online")->full_tablename() . " WHERE t2.org_name = '" . $orgname . "'");
		if ($pcount[0][0] == 0)
