...
class OnlineDisplay extends BaseActiveModule
{
	...
	*/
	function online_list($channel, $like, $lvl = FALSE)
	{
		...
		$online = $this->bot->db->select("SELECT t1.nickname, t2.level, org_rank, org_name, " . $this->cp . $ex2 . ", t1.level FROM " . "#___online AS t1 LEFT JOIN #___whois AS t2 ON t1.nickname = t2.nickname WHERE status_" . $channel . "=1" . $andlvl . " AND " . $botstring . $like . $sortstring);
	...
	*/
	function sm_msg($what)
	{
		...
		$online = $this->bot->db->select("SELECT DISTINCT(t1.nickname), t2.level, " . $this->cp . $ex1 . ", org_name FROM " . $this->bot->core("online")->full_tablename() . " WHERE t2.level >= 1" . " ORDER BY t1.nickname ASC, " . $this->cp . " ASC, t2.level DESC");
		$count = 0;
