...
class Glyph extends BaseActiveModule
{
	...
	function start_glyph($name, $a, $e, $o)
	{
		if ($this->bot->core("security")->check_access($name, "leader"))
		{
			if (! $this->raffle)
			{
				...
				$this->bot->send_pgroup("##highlight##$name##end## has ##highlight##started##end## " . "a raffle for ##highlight##$a##end## Aban glyphs, " . "##highlight##$e##end## Enel glyphs, " . "##highlight##$o##end## Ocra glyphs. :: " . $this->click_join() . "\n##highlight##" . " - Glyphs are for your own use only. You may NOT sell them!\n" . " - A won glyph will cost you 2 raidpoints.##end##");
			}
			else
	...
	*/
	function join_glyph($name)
	{
		if ($this->raffle)
		{
			$result = $this->bot->db->select("SELECT points FROM raid_points WHERE id = " . $this->bot->core("chat")->get_uid($name));
			if ($result[0][0] < 2)
	...
	*/
	...
	{
		$result = $this->bot->db->select("SELECT profession FROM users WHERE id = " . $this->bot->core("chat")->get_uid($name));
		$result = $result[0][0];
		if (($result == "Enforcer") || ($result == "Engineer") || ($result == "Keeper") || ($result == "Martial Artist") || ($result == "Shade"))
			return "aban";
		else if (($result == "Agent") || ($result == "Bureaucrat") || ($result == "Nano-Technician") || ($result == "Soldier"))
			return "enel";
		else
			return "ocra";
	}
