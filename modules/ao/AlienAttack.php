	...
	function gmsg($name, $group, $msg)
	{
		if ($name == "0")
		{
			...
			else if (preg_match("/(.+) turned the cloaking device in your city off./i", $msg, $info))
			{
				...
				{
					$this->bot->core("timer")->add_timer(false, $player, 60 * 60 + 1, $this->bot->core("shortcuts")->get_short($this->bot->guildname) . "'s cloak can be enabled again", "gc", 0, "CityWarning");
				}
			...
			else if (preg_match("/(.+) turned the cloaking device in your city on./i", $msg, $info))
			{
				...
				if ($this->bot->core("settings")->get("AlienAttack", "PublicTimer"))
				{
					$this->bot->core("timer")->add_timer(false, $player, 60 * 60 + 1, $this->bot->core("shortcuts")->get_short($this->bot->guildname) . "'s cloak can be disabled again", "gc", 0, "CityWarning");
				}
