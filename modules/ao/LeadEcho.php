...
class LeadEcho extends BaseActiveModule
{
  ...
  */
  ...
  function set_leader($caller, $leadername, $repeatstring)
  {
    ...
    if ($this->bot->core("settings")->get("Leader", "Name") != '' && ($this->bot->core("security")->get_access_level($caller) < $this->bot->core("security")->get_access_level($this->bot->core("settings")->get("Leader", "Name")))) {
      return "##error##" . $caller . ", you can't take lead from " . $this->bot->core("settings")->get("Leader", "Name") . "##end##";
    }
