        ...
        switch ($userlevel)
        {
            case 2:
                if (strtolower($this->bot->core("settings")
                    ->get("Members", "AutoInviteGroup")) == 'members' || strtolower($this->bot
                    ->core("settings")
            ...
            default:
                if (strtolower($this->bot->core("settings")
                    ->get("Members", "AutoInviteGroup")) == 'guests' || strtolower($this->bot
                    ->core("settings")
                    ->get("Members", "AutoInviteGroup")) == 'both'
                ) {
                    return true;
    ...
    function notify($user, $startup = false)
    {
        ...
        if ($this->bot->core("settings")->get("Autoinv", "Activated")) {
            if ($this->bot->core('prefs')
                    ->get($user, 'AutoInv', 'receive_auto_invite') == 'On' && $this->check_access($user) && !($this->bot
