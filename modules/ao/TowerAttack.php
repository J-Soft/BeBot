...
class TowerAttack extends BaseActiveModule
{
	...
	function update_table()
	{
		...
		{
			$this->bot->db->set_version("tower_result", $this->bot->core("settings")->get("TowerAttack", "SchemaVersion"));
			$this->bot->db->set_version("tower_attack", $this->bot->core("settings")->get("TowerAttack", "SchemaVersion"));
		...
		switch ($this->bot->db->get_version("tower_result"))
		{
			case 1:
				...
				$this->bot->db->update_table("tower_result", array("time" , "win_guild" , "win_side" , "lose_guild" , "lose_side" , "zone"), "alter", "ALTER IGNORE TABLE #___tower_result " . "ADD UNIQUE (time, win_guild, win_side, lose_guild, lose_side, zone), " . "ADD INDEX (win_guild), ADD INDEX (win_side), ADD INDEX (zone)");
		}
	...
	*/
	function victory_blob()
	{
		...
		$result = $this->bot->db->select("SELECT time, win_guild as off_guild, win_side as off_side, lose_guild as def_guild," . " lose_side as def_side, zone FROM #___tower_result ORDER BY time DESC LIMIT 0, " . $this->bot->core("settings")->get("TowerAttack", "VictoryBlobSize"), MYSQL_ASSOC);
		if (empty($result))
		{
		...
		{
			$battle .= "##blob_text##" . $this->format_attack_string($res, $this->bot->core("settings")->get("TowerAttack", "VictoryString"), true) . "</font>\n\n";
		}
	...
	*/
	function battle_blob()
	{
		...
		$result = $this->bot->db->select("SELECT time, off_guild, off_side, off_player, off_level, off_profession, " . "def_guild, def_side, zone, x_coord, y_coord FROM #___tower_attack ORDER BY time DESC LIMIT 0, " . $this->bot->core("settings")->get("TowerAttack", "AttacksPerBlob"), MYSQL_ASSOC);
		if (empty($result))
		{
	...
	*/
	function gmsg($name, $group, $msg)
	{
		...
		if ($attack)
		{
			...
			{
				$player = array('level' => '0' , 'profession' => 'Unknown' , 'off_side' => 'Unknown');
			}
			else
			{
			...
			if ($this->bot->core("settings")->get("TowerAttack", "Spam"))
			{
				...
				if ($this->bot->core("settings")->get("TowerAttack", "SpamPrevention") && ($this->suppress[$infos["def_guild"]] + $spamtime) > time())
				{
				...
				else
				{
					if ($infos["off_guild"] != "")
					{
						$msg = $this->format_attack_string($infos, $this->bot->core("settings")->get("TowerAttack", "AttackStringOrged"));
					}
					else
					{
						$msg = $this->format_attack_string($infos, $this->bot->core("settings")->get("TowerAttack", "AttackStringUnOrged"));
					}
					$this->suppress[$infos["def_guild"]] = time();
	...
	function relay_tower_damage($tower, $zone, $health, $attacker = "", $org = "")
	{
		...
		if (strtolower($this->bot->core("settings")->get("TowerAttack", "RelayTowerDamage")) == "both" || strtolower($this->bot->core("settings")->get("TowerAttack", "RelayTowerDamage")) == "relay")
		{
			$this->bot->core("relay")->relay_to_bot($msg);
		}
		if (strtolower($this->bot->core("settings")->get("TowerAttack", "RelayTowerDamage")) == "both" || strtolower($this->bot->core("settings")->get("TowerAttack", "RelayTowerDamage")) == "pgroup")
		{
